...
import { AppStateMachine, APPURL, HTTPCode } from "../../Utils";
import { PageResponse, ResponseData } from "../types";
...
const cmsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(updatePageAsync.pending, (state, action) => {
            state.state = AppStateMachine.Pending;
        });

        builder.addCase(updatePageAsync.rejected, (state, action) => {
            state.state = AppStateMachine.Error;
        });

        builder.addCase(resetPageAsync.pending, (state, action) => {
            state.state = AppStateMachine.Pending;
        });

        builder.addCase(resetPageAsync.rejected, (state, action) => {
            state.state = AppStateMachine.Error;
        });

        builder.addCase(updatePageAsync.fulfilled, (state, action) => {
            if (action.payload.code == HTTPCode.OK) {
                state.state = AppStateMachine.Completed;
                const data = action.payload.data as PageResponse
                state.webPagesState = JSON.parse(data.content);
            }
        });

        builder.addCase(resetPageAsync.fulfilled, (state, action) => {
            if (action.payload.code == HTTPCode.OK) {
                state.state = AppStateMachine.Completed;
                const data = action.payload.data as PageResponse
                state.webPagesState = JSON.parse(data.content);
        ...
        builder.addCase(getPageAsync.fulfilled, (state, action) => {
            if (action.payload.code == HTTPCode.OK) {
                state.state = AppStateMachine.Completed;
                const data = action.payload.data as PageResponse
                // console.log(data);
                state.webPagesState = JSON.parse(data[0].content);
            }
        });
        builder.addCase(getPageAsync.pending, (state, action) => {
            state.state = AppStateMachine.Pending;
        });

        builder.addCase(getPageAsync.rejected, (state, action) => {
            state.state = AppStateMachine.Error;
        });
    }
});

export const getPageAsync = createAsyncThunk('getWebContent', async () => {
    const response = await CMSAPI.getPage();
    console.log(response);
    return response.data;
});

export const resetPageAsync = createAsyncThunk('resetWebContent', async () => {
    const response = await CMSAPI.savePage(initialState.webPagesState) as AxiosResponse<ResponseData>;
    return response.data;
});

export const updatePageAsync = createAsyncThunk('saveWebContent', async (webState: PageContent) => {
    const response = await CMSAPI.savePage(webState) as AxiosResponse<ResponseData>;
    return response.data;
})
